...
/**
 * @brief   Dessine un logo "Å“il dans un triangle" centrÃ© sur l'Ã©cran LCD.
 *
 * @details Le logo est centrÃ© automatiquement selon DISPLAY_WIDTH et DISPLAY_HEIGHT.
 *          Les tailles sont adaptÃ©es pour un Ã©cran 128x64.
 */
void DisplayEyeLogo(void) {
    ...
    //7064464
    // PaupiÃ¨re supÃ©rieure (triangle noir)
    int lidOffsetY = 7;
...
void SetSignalName(int index, const char* newName) {
    if (index >= 0 && index < 7) {
        ...
        signalNames[index][sizeof (signalNames[index]) - 1] = '\0'; // SÃ©curitÃ©
    }
}
